...
import { Shield, FileText, Rocket, Megaphone, CheckSquare, BookOpen, MessageSquare, Eye, EyeOff, User, LogOut } from 'lucide-react';
...
export default function BackofficePage({ params }: PageProps) {
  ...
  const visibleTabs = showClientView
    ? SHEET_DEFINITIONS.filter(sheet => sheet.visibleToClient)
    : SHEET_DEFINITIONS;
  ...
  const tabIcons: Record<string, any> = {
    ...
    'kyc-documents': CheckSquare,
    'csm-overview': BookOpen,
  ...
  const getOverallProgress = () => {
    const clientSheets = SHEET_DEFINITIONS.filter(s => s.visibleToClient);
    let totalCompleted = 0;
    let totalFields = 0;

    clientSheets.forEach(sheet => {
      const progress = getSheetProgress(sheet.name);
      totalCompleted += progress.completed;
      totalFields += progress.total;
    });

    return { completed: totalCompleted, total: totalFields };
  };
      ...
      <aside className="w-72 border-r border-border bg-muted/30 flex flex-col">
        ...
        <div className="p-6 border-b border-border">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <div className="font-semibold text-foreground">Skipr Backoffice</div>
          ...
          <button
            ...
            className="w-full mt-4 flex items-center justify-center gap-2 px-3 py-2 bg-secondary hover:bg-secondary/80 rounded-lg transition-colors text-sm font-medium"
          >
            {showClientView ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
            {showClientView ? 'Client View' : 'Full View'}
          </button>
        </div>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-auto">
          ...
          {visibleTabs.filter(t => t.visibleToClient).map((tab) => {
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-5xl mx-auto px-8 py-12">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {!activeSheet?.visibleToClient && (
                    <span className="px-2 py-1 bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400 rounded-md text-xs font-medium">
                      Internal Only
                    </span>
                  )}
                </div>
                <p className="text-muted-foreground mt-1">
                  {activeSheet?.visibleToClient
                    ? 'Client-facing section - visible to the customer'
                    : 'Internal section - not visible to the customer'
                  }
                </p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            {/* Show GroupManager for mobility programs */}
            {activeTab === 'mobility-programs' && (
          ...
          <div className="mt-8 p-4 bg-accent/50 border border-border rounded-lg text-sm text-muted-foreground flex items-start gap-3">
            ...
            <p>
              Internal team view - You have access to all tabs including client-hidden sections. Toggle between "Client View" and "Full View" in the sidebar to preview what the client sees.
            </p>
          </div>
        </div>
      </main>
